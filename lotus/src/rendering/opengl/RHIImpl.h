...
TextureID GLCreateTexture(const TextureInfo &info)
{
    ...
    glTexImage2D(
        GL_TEXTURE_2D,
        0,
        GLStorageFormatFromEnum(info.InternalFormat),
        info.Width,
        info.Height,
        0,
        GLStorageFormatFromEnum(info.Format),
        GLDataTypeFromEnum(info.DataType),
        info.Data);
...
{
    if (face == RHI::CULL_FACE_BACK)
        glCullFace(GL_BACK);
    else if (face == RHI::CULL_FACE_FRONT)
        glCullFace(GL_FRONT);
    else
        glCullFace(GL_BACK);
}
...
inline void GLAttachRenderBuffer(FrameBuffer fb, const RenderBufferAttachmentInfo &info)
{
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GLStorageFormatFromEnum(info.InternalFomat), info.Width, info.Height);
...
{
    if (bits & ENABLE_DEPTH_TEST)
        glEnable(GL_DEPTH_TEST);
    if (bits & ENABLE_CULL_FACE)
