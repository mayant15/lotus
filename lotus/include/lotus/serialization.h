...
{
    explicit OutputArchive(entt::registry *reg) : impl(reg), snapshot(*reg)
    {
    }

    template <class Component> void components()
    {
  ...
  protected:
    ...
    struct impl
    {
      ...
      public:
        explicit impl(entt::registry *reg) : pReg(reg)
        {
        }
        ...
        template <class Component> void operator()(entt::entity e, Component comp)
        {
